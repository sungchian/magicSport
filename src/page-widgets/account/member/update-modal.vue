...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onAfterShown = async () => {
      ...
      form.isArticleAudit = props.data.isArticleAudit;
    ...
    const onSubmit = async (done) => {
      ...
      form.birthday =
        form.birthday.substr(-1) === "Z"
          ? form.birthday
          : form.birthday + "T00:00:00Z";

      const res = await store.dispatch("account/member/update", form);

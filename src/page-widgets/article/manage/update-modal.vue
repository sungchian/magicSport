...
export default {
  ...
  setup(props, { emit }) {
    ...
    setPrefix({ $current: "pages.article.manage.form" });
    ...
    const onAfterShown = async () => {
      // console.log("dataProp", filter(propEq("tagType", 1), props.data.tag));
      ...
      //approver跟 publisher 都用id存
      form.approver = props.data.approverId;
      ...
      form.publisher = props.data.publisherId;
      ...
      //   : [];
      form.tagTeam = props.data.tag
        ? filter(propEq("tagType", 2), props.data.tag)
        : [];
      form.tagPlayer = props.data.tag
        ? filter(propEq("tagType", 1), props.data.tag)
        : [];
    ...
    const onSubmit = async (done) => {
      ...
      console.log("team", form.tagTeam);
      form.tag = concat(form.tagTeam, form.tagPlayer);
      form.tag = forEach((tag) => {
        omit(["name_en", "name_zh", "name_zht"], tag);
      }, form.tag);
      console.log("2", form.tag);
      const res = await store.dispatch(
        "article/manage/update",
        omit(["tagPlayer", "tagTeam"], form)
      );
      ...
      store.commit("app/set/alert", {
        ...
        message: t("feedback.update.success.with.describe", {
          describe: t("$current.update.success", { name: form.shortContent }),
        }),
      });
    ...
    const searchForTeamTag = async (input) => {
      ...
      if (tagType.team === 1) {
        ...
        if (res.result === null) {
          ...
          // res.result =>
          // id: 24435
          // name_en: "FK Krasnodar-2000"
          // name_zh: "克拉斯诺达尔-2000"
          // name_zht: "克拉斯诺达尔-2000"
          // sportType: 1
          // tagType: 2
          options.tagTeam = map(
            (tag) => ({ value: tag, children: tag.name_zht }),
            res.result
          );
        }
      ...
      if (tagType.player === 1) {
        ...
        if (res.result === null) {
          ...
          options.tagPlayer = map(
            (tag) => ({ value: tag, children: tag.name_en }),
            res.result
          );
        }

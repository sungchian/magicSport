...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onAfterShown = async () => {
      ...
      form.articleId = props.data.articleId;
      form.categoryId = props.data.categoryId;
      form.link = props.data.link;
      form.kind = props.data.kind;
      form.picUrl = props.data.picUrl;
      form.title = props.data.title;
      form.releaseDatetime = props.data.releaseDatetime;
      form.expirationDatetime = props.data.expirationDatetime;
    };
    ...
    const onSubmit = async (done) => {
      ...
      if (form.releaseDatetime && form.releaseDatetime.substr(-1) !== "Z") {
        form.releaseDatetime = form.releaseDatetime.replace(" ", "T") + "Z";
      }
      if (
        form.expirationDatetime &&
        form.expirationDatetime.substr(-1) !== "Z"
      ) {
        form.expirationDatetime =
      ...
      const res = await store.dispatch("article/slide/update", form);
      ...
      store.commit("app/set/alert", {
        ...
        message: t("feedback.update.success.with.describe", {
          describe: t("$current.update.success", { name: form.title }),
        }),
      });
